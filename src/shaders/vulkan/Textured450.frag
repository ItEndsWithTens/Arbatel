...
layout (set = 3, binding = 0) uniform sampler point;

layout (set = 4, binding = 0) uniform texture2D tex;

layout (constant_id = 0) const bool flip_vertical = false;

void main()
{
	fsout_color = texture(sampler2D(tex, point), fsin_texcoords) * fsin_color;
